...
export function ModelSelector({ models: propModels, model: propModel, onModelChange }: ModelSelectorProps = {}) {
  ...
  // Need either a selected model or models to choose from
  if (!model && !models.length) return null

  const displayLabel = model ? formatModelName(model) : "Default"
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className={`text-[11px] font-medium ${model ? "text-fg" : "text-fg-muted"}`}>{displayLabel}</span>
        {models.length > 0 && (
          <svg
